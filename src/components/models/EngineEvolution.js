    ...
    <div className="grid grid-cols-4 overflow-hidden rounded-md border border-[var(--color-border)] bg-[var(--color-surface-raised)] shadow-[0_8px_24px_var(--color-shadow)]">
      {items.map((item) => (
        ...
        >
          ...
          <strong className="mt-1.5 text-[13px] leading-tight text-[var(--color-text)] md:text-[15px]">
            {item.title}
          </strong>
          <span className="mt-0.5 text-[12px] leading-tight text-[var(--color-text-muted)] md:text-[13px]">
            {item.text}
          </span>
        </div>
      ))}
    ...
    <div className="flex flex-wrap justify-center gap-4">
      {items.map((engine) => (
        <div key={engine} className="flex min-w-[68px] flex-col items-center">
          <span className="relative h-[4.75rem] w-[4.75rem] md:h-[5.25rem] md:w-[5.25rem]">
            <Image src={ENGINE_IMAGE} alt="" fill className="object-contain" sizes="84px" />
          </span>
          <strong
            className="mt-1 text-center text-[13px] text-[var(--color-text)] md:text-[15px]"
...
function DesktopTable({ eras, columns, isDark }) {
  return (
    <div className="hidden overflow-hidden rounded-md border border-[var(--color-border)] bg-[var(--color-surface-raised)] shadow-[0_8px_24px_var(--color-shadow)] md:block">
      <div className="grid grid-cols-[19%_10%_26%_22%_23%] bg-black text-[14px] font-bold text-white md:text-[15px] dark:bg-[var(--color-chrome)] dark:text-[var(--color-page)]">
        {(columns || []).map((column) => (
        ...
        return (
          ...
          >
            <div className="border-r border-[var(--color-border)] px-4 py-3 md:px-5 md:py-4">
              ...
              <div className="relative mt-2 h-[110px] w-full overflow-hidden rounded-md md:h-[140px]">
      ...
      {eras.map((era, index) => {
        ...
        return (
          ...
          >
            ...
            >
              <span className="relative h-[84px] w-[120px] shrink-0 overflow-hidden rounded-md">
                <Image src={RIGHT_IMAGE} alt="" fill className="object-cover object-center" sizes="120px" />
              </span>
                ...
                <div className="grid grid-cols-[36%_64%] border-b border-[var(--color-border)]">
                  <div className="flex items-center gap-3 border-r border-[var(--color-border)] p-4 font-bold">
                    <span className="relative h-12 w-12 shrink-0">
                      <Image src={ENGINE_IMAGE} alt="" fill className="object-contain" sizes="48px" />
                    </span>
                    Key Engines
                  </div>
                  <div
                    className={`p-4 ${sectionTableText} text-[var(--color-text)]`}
                    dangerouslySetInnerHTML={{ __html: cleanText(era.keyEngines) }}
                  />
                </div>
                <div className="grid grid-cols-[36%_64%] border-b border-[var(--color-border)]">
                  <div className="flex items-center gap-3 border-r border-[var(--color-border)] p-4 font-bold">
                    <HomeIcon name="timeline" isDark={isDark} className="h-8 w-8 object-contain" />
                    Why It Changed
                  </div>
                  <div
                    className={`p-4 leading-[1.45] text-[var(--color-text)] ${sectionTableText}`}
                    dangerouslySetInnerHTML={{ __html: cleanText(era.whyBmwChanged) }}
                  />
                </div>
                <div className="grid grid-cols-[36%_64%]">
                  <div className="flex items-center gap-3 border-r border-[var(--color-border)] p-4 font-bold">
                    <HomeIcon name="note" isDark={isDark} className="h-8 w-8 object-contain" />
                    Worth Knowing
                  </div>
                  <div
                    className={`p-4 leading-[1.45] text-[var(--color-text)] ${sectionTableText}`}
