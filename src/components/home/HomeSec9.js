  ...
  return (
    <div className="mb-3 border border-gray-200">
      ...
      {block.rows.map((row) => (
        ...
        >
          <span
            className="mb-1 text-sm font-bold text-blue-700 md:mb-0"
            dangerouslySetInnerHTML={{ __html: row.id }}
          />

          <p
            className="mb-1 text-sm font-semibold text-black md:mb-0"
          ...
          <p
            className={`mb-1 text-xs font-semibold md:mb-0 ${
              severityClass[row.severity.type] || ""
            }`}
          >
            <span
              dangerouslySetInnerHTML={{
                __html: `${row.severity.icon} ${row.severity.label}`,
              }}
            />
          </p>
